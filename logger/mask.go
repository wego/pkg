...
Numbers and booleans are masked through their literal text, at every depth; a
masked value is always written back as a string. Objects, arrays and null carry no
text to mask and are left untouched. Every member matching a key is masked, so a
duplicated key cannot leave one copy behind; members duplicating a key collapse
into one, masked from the value encoding/json would have bound, the last of them.
...
*/
func MaskJSON(json, maskChar string, toMasks []MaskData, opts ...Option) string {
	maskChar = maskCharOrDefault(maskChar)
	...
	for _, toMask := range toMasks {
		replaceLeaves(root, toMask.JSONKeys, options, func(v *fastjson.Value) *fastjson.Value {
			value, ok := scalarText(v)
			if !ok || value == "" {
				return nil
			}
			return jsonString(getMaskedValue(maskChar, value, toMask))
		})
	}
